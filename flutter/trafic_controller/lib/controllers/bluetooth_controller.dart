...
class BluetoothController extends GetxController {
  ...
  Get.defaultDialog(
    ...
    contentPadding: const EdgeInsets.all(20.0), // Content padding for inner spacing
    actions: [
      ...
      ElevatedButton(
        ...
        child: const Row(
          ...
          children:  [
            Icon(Icons.bluetooth, color: Colors.white, size: 20), // Bluetooth icon
            SizedBox(width: 8),
            Text(
              "Activer Bluetooth",
  ...
  Future<void> connectWithRetry(BluetoothDevice device) async {
    ...
    while (DateTime.now().difference(startTime).inSeconds < 2) {
      ...
      await Future.delayed(const Duration(milliseconds: 10 )); // Retry after 1 second
    } }
  ...
  Future<void> connectToDevice(BluetoothDevice device) async {
    try {
      final BluetoothConnection bluetoothConnection = await BluetoothConnection.toAddress(device.address);
    ...
    if (connection.value != null) {
      ...
      connection.value!.input!.listen((Uint8List data) {
        ...
        while (buffer.contains('\n')) {
          ...
          buffer = buffer.substring(newlineIndex + 1); // Remove processed message
  ...
  void _processCommand(String command) {
    command = command.replaceAll(RegExp(r'[^\w: ]'), ''); // Remove unexpected characters
    List<String> splittedCommand = command.trim().split(":");
    switch (splittedCommand[0]) {
      case "GG1":
        green1Value.value = int.parse(splittedCommand[1]);
        print("green 1 changed to ${int.parse(splittedCommand[1])}");
        break;
      case "GG2":
        green2Value.value = int.parse(splittedCommand[1]);
        print("green 2 changed to ${int.parse(splittedCommand[1])}");
        break;
      case "GY":
        yellowValue.value = int.parse(splittedCommand[1]);
        print("yellow changed to ${int.parse(splittedCommand[1])}");
        break;
      case "GR":
        redValue.value = int.parse(splittedCommand[1]);
        print("red changed to ${int.parse(splittedCommand[1])}");
        break;
      case "INM":
        activateNightMode.value = splittedCommand[1] == "0" ? false: true;
        print("active mode changed to ${int.parse(splittedCommand[1])}");
        break;
      default:
